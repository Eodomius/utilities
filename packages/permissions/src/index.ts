...
class Permissions {
  ...
  public find(permissions: string | string[]) {
    if (typeof permissions === "string" || typeof permissions === "number") {
      const permission = this.permissions.find(
        (f) =>
          f.name === permissions.toString().toUpperCase() ||
          f.value === permissions
      );
    ...
    if (Array.isArray(permissions)) {
      ...
      permissions.map((p) => {
        const permission = this.permissions.find(
          (perm) => perm.name === p.toString().toUpperCase() || perm.value === p
        );
   ...
   */
  public equals(
    other: string | number | Array<string | number>
  ...
  public addPermission(
    permissionToAdd: string | string[] | number
    ...
    if (type === "string") {
      const permission = this.permissions.find(
        (p) => p.name === permissionToAdd
      );
      const has: any = this.calculate().filter(
    ...
    return new TypeError(
      "The permission must be a string, an array or a number."
    );
  }
  ...
  public removePermission(
    permissionToRemove: string | string[] | number
  ): string | TypeError | boolean {
